...
**WHY THIS HAPPENS:** The `dist/` directory contains compiled TypeScript code
and is not stored on GitHub. It is normally generated by the package's
`prepare` script during `npm install`. If lifecycle scripts did not run or the
build failed, the HUD bundle will be missing.
