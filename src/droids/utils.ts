...
export function loadAgentPrompt(agentName: string): string {
  ...
  try {
    ...
    const agentPath = join(droidsDir, `${agentName}.md`);

    // Security: Verify resolved path is within the droids directory
    const resolvedPath = resolve(agentPath);
    const resolvedDroidsDir = resolve(droidsDir);
    const rel = relative(resolvedDroidsDir, resolvedPath);
...
/**
 * Build delegation table section for OMD prompt
 */
...
/**
 * Build key triggers section for OMD prompt
 */
