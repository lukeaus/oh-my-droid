 ...
 * - user_abort, abort_by_user, user_stop, stop_button, user_request: User-initiated stops
 * - abort, cancel, interrupt: Generic abort patterns (exact match only to prevent false positives)
 *
...
export function isUserAbort(context?: StopContext): boolean {
  ...
  // Check stop_reason and end_turn_reason patterns indicating user abort
...
function getTodoFilePaths(sessionId?: string, directory?: string): string[] {
  ...
  if (sessionId) {
    paths.push(join(factoryDir, 'sessions', sessionId, 'todos.json'));
    paths.push(join(factoryDir, 'todos', `${sessionId}.json`));
  }
