...
# Save
import pickle
df.to_pickle('.omd/scientist/state.pkl')

# Load (only if needed after timeout or in different session)
import pickle
df = pd.read_pickle('.omd/scientist/state.pkl')
"""
)
```
...
python_repl(
  ...
  code="""
# Save
df.to_parquet('.omd/scientist/state.parquet')

# Load
df = pd.read_parquet('.omd/scientist/state.parquet')
"""
)
```
...
with open('.omd/scientist/results.json', 'w') as f:
    json.dump(results, f)

# Load
import json
with open('.omd/scientist/results.json', 'r') as f:
    results = json.load(f)
"""
)
```
...
# Save
import pickle
with open('.omd/scientist/model.pkl', 'wb') as f:
    pickle.dump(model, f)

# Load
import pickle
with open('.omd/scientist/model.pkl', 'rb') as f:
    model = pickle.load(f)
"""
)
```
...
```python
# Instead of printing
df.to_csv('.omd/scientist/full_results.csv', index=False)
print("[FINDING] Full results saved to .omd/scientist/full_results.csv")
```
</Output_Management>
...
LOCATION: Save reports to `.omd/scientist/reports/{timestamp}_report.md`
...
report_dir = '.omd/scientist/reports'
...
LOCATION: Save all figures to `.omd/scientist/figures/{timestamp}_{name}.png`
...
os.makedirs('.omd/scientist/figures', exist_ok=True)
...
plt.tight_layout()
plt.savefig(f'.omd/scientist/figures/{timestamp}_bar_chart.png', dpi=150)
plt.close()
print(f"[FINDING] Bar chart saved to .omd/scientist/figures/{timestamp}_bar_chart.png")
"""
)
```
...
ax.set_ylabel('Value')
plt.tight_layout()
plt.savefig(f'.omd/scientist/figures/{timestamp}_line_chart.png', dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(f'.omd/scientist/figures/{timestamp}_scatter.png', dpi=150)
plt.close()
"""
)
```
...
plt.tight_layout()
plt.savefig(f'.omd/scientist/figures/{timestamp}_heatmap.png', dpi=150)
plt.close()
"""
)
```
...
plt.tight_layout()
plt.savefig(f'.omd/scientist/figures/{timestamp}_histogram.png', dpi=150)
plt.close()
"""
)
```
